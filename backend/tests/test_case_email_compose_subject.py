...
def test_email_compose_subject_uses_matter_title_not_precedent_name(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "app.case_email_compose_service.resolve_blank_email_compose_body",
        lambda _db, merge_in: merge_in,
    )
    monkeypatch.setattr(
        "app.case_email_compose_service.merge_compose_docx_bytes",
        lambda *_a, **_k: (b"docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"),
    )
    monkeypatch.setattr(
        "app.case_email_compose_service.extract_plain_text_from_docx_bytes",
        lambda _b: "Dear Client,\n\nBody text.",
    )
    monkeypatch.setattr("app.case_email_compose_service._resolve_recipient_email_m365", lambda *_a, **_k: "")
