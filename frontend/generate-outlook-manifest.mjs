...
const allowedJsonDir =
  target === 'public'
    ? path.join(frontendRoot, 'public', '.well-known')
    : path.join(frontendRoot, 'dist', '.well-known')
const allowedJsonPath = path.join(allowedJsonDir, 'microsoft-officeaddins-allowed.json')
if (fs.existsSync(allowedJsonPath) && origin) {
  const allowedBefore = fs.readFileSync(allowedJsonPath, 'utf8')
  const allowedAfter = rewriteManifestXml(allowedBefore, origin)
  if (allowedAfter !== allowedBefore) {
    fs.writeFileSync(allowedJsonPath, allowedAfter)
    console.log(`[outlook-addin] ${target}/.well-known/microsoft-officeaddins-allowed.json URLs → ${origin}`)
  }
}
